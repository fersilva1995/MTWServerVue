<template>
  <div>
    ...
    <b-form inline>
      ...
      <div class="control mb-2 mr-2 ml-2">
        <label for="hours">{{ $i18n.t('hours') }}</label>
        <input class="control" type="number" id="hours" autocomplete="off" v-model.lazy="element.hours" />
      </div>
    </b-form>
    ...
    update() {
      ...
      .then((element) => {
        if(this.element.id <= 0) {
          ...
          this.$emit('addAction', { row: this.row, value: this.element, update: false, element: new Element(i18n.t('newPeriod'))});
        } else {
          this.$emit('addAction', { row: this.row, value: this.element, update: true, element: new Element(i18n.t('newPeriod'))});
        }
